#!/usr/bin/env python3
"""Compile lean German lesson authoring data into Muhkam's baked exercise schema.

Input:  data/german-src/*.json — each file: { "level": "<levelId>", "lessons": [...] }
        A lesson is either:
          regular:  { number, title, titleNative?, topicId?, items: [{de, en, translit?}] }
          reading:  { number, title, titleNative?, context?, paragraphs: [{de, en}],
...
Output: data/courses-german.json — full course document, same shape as data/courses.json.

German uses the Latin alphabet plus umlauts (ä/ö/ü) and ß, so like
Spanish/French there is no alphabet-drill phase and no separate translit
field is needed for most content.

This course targets Standard German (Hochdeutsch), confirmed with the
user before writing any content, and -- unlike Spanish/French -- starts
at true A1 (beginner), confirmed with the user. Vocabulary is
cross-checked against the German Wiktionary dump on kaikki.org before
use, same discipline as this project's other courses. Audio uses a
"designed" (text-prompted) voice via Voicebox's tada engine, since no
dedicated German preset voice exists in Kokoro or any other Voicebox
engine at the time this course was built -- confirmed by checking every
engine's preset list before falling back to this approach.
"""
import json
import random
import re
...
SRC_DIR = ROOT / "data" / "german-src"
OUT_PATH = ROOT / "data" / "courses-german.json"

random.seed(20260828)  # deterministic distractor shuffling across runs

LEVEL_ORDER = ["a1", "a2", "b1", "b1plus", "b2", "b2plus", "c1", "c2"]
LEVEL_META = {
    "a1": dict(cefr="A1", label="Foundations", labelNative="Grundlagen"),
    "a2": dict(cefr="A2", label="Building Blocks", labelNative="Bausteine"),
    "b1": dict(cefr="B1", label="Everyday Fluency", labelNative="Alltagssicherheit"),
    "b1plus": dict(cefr="B1+", label="Expanding Range", labelNative="Erweiterter Ausdruck"),
    "b2": dict(cefr="B2", label="Complex Structures", labelNative="Komplexe Strukturen"),
    "b2plus": dict(cefr="B2+", label="Precision & Nuance", labelNative="Präzision und Nuance"),
    "c1": dict(cefr="C1", label="Advanced", labelNative="Fortgeschritten"),
    "c2": dict(cefr="C2", label="Mastery", labelNative="Meisterschaft"),
}
...
def compile_regular_lesson(level, lesson, en_pool_by_level, word_pool):
    ...
    for i, item in enumerate(items):
        ...
        if i % 3 == 0:
            ...
            exercises.append({"type": "listening-tap", "native": de, "answer": tokens})
    ...
    for start in range(0, len(items) - 3, 4):
        ...
        exercises.append({
            "type": "matching",
            "pairs": [{"native": c["de"], "en": c["en"]} for c in chunk],
        })

    out = {
        "id": f"de-{level}-{lesson['number']}",
...
def compile_reading_lesson(level, lesson):
    return {
        "id": f"de-{level}-{lesson['number']}-reading",
        "number": lesson["number"],
        "title": lesson["title"],
        ...
        "readingPassage": {
            ...
            "paragraphs": [{"native": p["de"], "en": p["en"]} for p in lesson["paragraphs"]],
        },
        "exercises": [
            {
                "type": "comprehension",
...
def main():
    ...
    course = {
        "id": "german",
        "title": "German, Start to Fluent",
        "subtitle": "Standard German from true beginner through fluent, everyday command — real sentences, real grammar, real conversation",
        "dir": "ltr",
        "lang": "de",
        "languageName": "German",
        "fontNative": "'Inter', 'Noto Sans', sans-serif",
        "flag": "Deutsch",
        "heroEyebrow": "Hochdeutsch · A1 →",
        "heroNative": "Vom Anfang bis zur Sicherheit",
        "heroLedeSuffix": "Real sentences from lesson one, with genuine native-quality audio for every single line.",
        "uiStrings": {"wordHoard": "Wortschatz", "review": "Wiederholung", "revision": "Übung"},
