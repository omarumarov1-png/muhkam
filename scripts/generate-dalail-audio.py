#!/usr/bin/env python3
"""TTS sweep for Dalail al-Khayrat: finds every Arabic item used across
data/dalail-src/*.json that isn't already in data/audio-dalail/manifest.json,
generates it via the local Voicebox server (designed-ar-dalail-reciter /
qwen -- a dedicated reverent, deep-male, tarteel-style recitation voice,
distinct from the casual conversational voices used for the dialect
courses), and appends it to the manifest as it goes. Re-run any time -- it
always rescans from the source files, so it naturally picks up newly
authored lessons without any bookkeeping.
"""
import hashlib
import json
import subprocess
...
AUDIO_DIR = ROOT / "data/audio-dalail"
MANIFEST_PATH = AUDIO_DIR / "manifest.json"

VOICEBOX = "http://localhost:17493"
PROFILE_ID = "7e02a1cf-69c2-4a91-8937-8ce5e889c2a4"  # designed-ar-dalail-reciter
...
def main():
    ...
    for i, text in enumerate(missing, 1):
        ...
        out_mp3 = AUDIO_DIR / f"dal_{h}.mp3"
        ok = False
