#!/usr/bin/env python3
"""Overnight TTS sweep for Emirati Arabic (Khaleeji): finds every Arabic
sentence used across data/emirati-src/*.json (regular-lesson items +
reading-lesson paragraphs) that isn't already in
data/audio-emirati/manifest.json, generates it via the local Voicebox
server (designed-ar-emirati-female / qwen), and appends it to the
manifest as it goes. Re-run any time -- it always rescans from the source
files, so it naturally picks up newly authored lessons without any
bookkeeping.
"""
import hashlib
import json
...
AUDIO_DIR = ROOT / "data/audio-emirati"
MANIFEST_PATH = AUDIO_DIR / "manifest.json"
LEVELS = ["a1", "a2", "b1", "b1plus", "b2", "b2plus", "c1", "c2"]

VOICEBOX = "http://localhost:17493"
PROFILE_ID = "6757dae2-d4e6-445b-b91b-eee2e963c91a"  # designed-ar-emirati-female
VOICE_NAME = "designed-ar-emirati-female"
...
def main():
    ...
    for i, text in enumerate(missing, 1):
        ...
        out_mp3 = AUDIO_DIR / f"afb_{h}.mp3"
        ok = False
