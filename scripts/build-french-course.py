#!/usr/bin/env python3
"""Compile lean French lesson authoring data into Muhkam's baked exercise schema.

Input:  data/french-src/*.json — each file: { "level": "<levelId>", "lessons": [...] }
        A lesson is either:
          regular:  { number, title, titleNative?, topicId?, items: [{fr, en, translit?}] }
          reading:  { number, title, titleNative?, context?, paragraphs: [{fr, en}],
...
Output: data/courses-french.json — full course document, same shape as data/courses.json.

French uses the Latin alphabet, so like Spanish there is no alphabet-drill
phase and no separate translit field is needed for most content.

This course targets Metropolitan (France) French, confirmed with the user
before writing any content. Vocabulary is cross-checked against the French
Wiktionary dump on kaikki.org before use, same discipline as this
project's other courses. Audio is genuine, dedicated French TTS (Kokoro
engine via the local Voicebox app, voice ff_siwis -- the only real French
preset voice available across every Voicebox engine, so unlike Spanish
there is no voice alternation) -- real neural French speech, not a
cross-language TTS compromise, so listening/listening-tap exercises ARE
generated here.
"""
import json
import random
...
SRC_DIR = ROOT / "data" / "french-src"
OUT_PATH = ROOT / "data" / "courses-french.json"

random.seed(20260828)  # deterministic distractor shuffling across runs

LEVEL_ORDER = ["a1", "a2", "b1", "b1plus", "b2", "b2plus", "c1", "c2"]
LEVEL_META = {
    "a1": dict(cefr="A1", label="Foundations", labelNative="Fondamentaux"),
    "a2": dict(cefr="A2", label="Building Blocks", labelNative="Bases essentielles"),
    "b1": dict(cefr="B1", label="Everyday Fluency", labelNative="Aisance au quotidien"),
    "b1plus": dict(cefr="B1+", label="Expanding Range", labelNative="Élargir son registre"),
    "b2": dict(cefr="B2", label="Complex Structures", labelNative="Structures complexes"),
    "b2plus": dict(cefr="B2+", label="Precision & Nuance", labelNative="Précision et nuance"),
    "c1": dict(cefr="C1", label="Advanced", labelNative="Avancé"),
    "c2": dict(cefr="C2", label="Mastery", labelNative="Maîtrise"),
}
...
def compile_regular_lesson(level, lesson, en_pool_by_level, word_pool):
    ...
    for i, item in enumerate(items):
        ...
        if i % 3 == 0:
            ...
            exercises.append({"type": "listening-tap", "native": fr, "answer": tokens})
    ...
    for start in range(0, len(items) - 3, 4):
        ...
        exercises.append({
            "type": "matching",
            "pairs": [{"native": c["fr"], "en": c["en"]} for c in chunk],
        })

    out = {
        "id": f"fr-{level}-{lesson['number']}",
...
def compile_reading_lesson(level, lesson):
    return {
        "id": f"fr-{level}-{lesson['number']}-reading",
        "number": lesson["number"],
        "title": lesson["title"],
        ...
        "readingPassage": {
            ...
            "paragraphs": [{"native": p["fr"], "en": p["en"]} for p in lesson["paragraphs"]],
        },
        "exercises": [
            {
                "type": "comprehension",
...
def main():
    ...
    course = {
        "id": "french",
        "title": "French, Start to Fluent",
        "subtitle": "Metropolitan French from real-world basics through fluent, everyday command — real sentences, real grammar, real conversation",
        "dir": "ltr",
        "lang": "fr",
        "languageName": "French",
        "fontNative": "'Inter', 'Noto Sans', sans-serif",
        "flag": "Français",
        "heroEyebrow": "Metropolitan French · A2 →",
        "heroNative": "De l'essentiel à l'aisance",
        "heroLedeSuffix": "Real sentences from lesson one, with genuine native-quality audio for every single line.",
        "uiStrings": {"wordHoard": "Vocabulaire", "review": "Révision", "revision": "Pratique"},
