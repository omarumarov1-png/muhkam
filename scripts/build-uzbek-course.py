#!/usr/bin/env python3
"""Compile lean Uzbek lesson authoring data into Muhkam's baked exercise schema.

Input:  data/uzbek-src/*.json — each file: { "level": "<levelId>", "lessons": [...] }
        A lesson is either:
          regular:  { number, title, titleNative?, topicId?, items: [{uz, en, translit?}] }
          reading:  { number, title, titleNative?, context?, paragraphs: [{uz, en}],
...
Output: data/courses-uzbek.json — full course document, same shape as data/courses.json.

Modern Uzbek uses the Latin alphabet (official since the 1990s), so unlike
Hebrew/Arabic there is no alphabet-drill a0 phase and unlike Kazakh there is
no separate translit field needed for most content -- the native script IS
already Latin.
"""
import json
import random
...
SRC_DIR = ROOT / "data" / "uzbek-src"
OUT_PATH = ROOT / "data" / "courses-uzbek.json"

random.seed(20260720)  # deterministic distractor shuffling across runs

LEVEL_ORDER = ["a1", "a2", "b1", "b1plus", "b2", "b2plus", "c1", "c2"]
LEVEL_META = {
    "a1": dict(cefr="A1", label="Foundations", labelNative="Asoslar"),
    "a2": dict(cefr="A2", label="Building Blocks", labelNative="Qurilish bloklari"),
    "b1": dict(cefr="B1", label="Everyday Fluency", labelNative="Kundalik ravonlik"),
    "b1plus": dict(cefr="B1+", label="Expanding Range", labelNative="Doirani kengaytirish"),
    "b2": dict(cefr="B2", label="Complex Structures", labelNative="Murakkab tuzilmalar"),
    "b2plus": dict(cefr="B2+", label="Precision & Nuance", labelNative="Aniqlik va nozik farqlar"),
    "c1": dict(cefr="C1", label="Advanced", labelNative="Yuqori daraja"),
    "c2": dict(cefr="C2", label="Mastery", labelNative="Yuksak mahorat"),
}
...
def compile_regular_lesson(level, lesson, en_pool_by_level, word_pool):
    ...
    for start in range(0, len(items) - 3, 4):
        ...
        exercises.append({
            "type": "matching",
            "pairs": [{"native": c["uz"], "en": c["en"]} for c in chunk],
        })

    out = {
        "id": f"uz-{level}-{lesson['number']}",
...
def compile_reading_lesson(level, lesson):
    return {
        "id": f"uz-{level}-{lesson['number']}-reading",
        "number": lesson["number"],
        "title": lesson["title"],
        ...
        "readingPassage": {
            ...
            "paragraphs": [{"native": p["uz"], "en": p["en"]} for p in lesson["paragraphs"]],
        },
        "exercises": [
            {
                "type": "comprehension",
...
def main():
    ...
    course = {
        "id": "uzbek",
        "title": "Uzbek, Start to Fluent",
        "subtitle": "Modern Uzbek from absolute beginner through fluent, everyday command — real sentences, grammar, and conversational practice",
        "dir": "ltr",
        "lang": "uz",
        "languageName": "Uzbek",
        "fontNative": "'Inter', 'Noto Sans', sans-serif",
        "flag": "Oʻzbekcha",
        "heroEyebrow": "Modern Uzbek · A1 → C1",
        "heroNative": "Asoslardan erkin so'zlashuvgacha",
        "heroLedeSuffix": "Real sentences from lesson one, built the same way as this app's Kazakh and Tajik courses.",
        "uiStrings": {"wordHoard": "So'z boyligi", "review": "Takrorlash", "revision": "Mashq"},
